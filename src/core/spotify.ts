...
export class SpotifyClient {
  ...
  private async ensureToken() {
    ...
    if (__sharedTokenInflight) {
      await __sharedTokenInflight; // copy over after wait
      if (this.cfg.accessToken && this.cfg.tokenExpiresAt && Date.now() < this.cfg.tokenExpiresAt - 60_000) return;
    ...
    __sharedTokenInflight = this.tokenInflight = (async () => {
      ...
      let externalEndpoint = await env('SPOTIFY_TOKEN_ENDPOINT');
      console.log('🔗 Environment check:', {
        SPOTIFY_TOKEN_ENDPOINT: externalEndpoint
      });
      if (externalEndpoint) {
