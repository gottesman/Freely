...
export function useArtistBuckets() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      //frontendLogger.log('🎵 useArtistBuckets: Loading data for artist ID:', deferredArtistId);
      const w: any = window;
      setBuckets(b => ({ ...b, loading: true, error: undefined, fetched: false }));
      try {
        let albumsResp: any;
        if (w.electron?.spotify?.getArtistAlbums) {
          //frontendLogger.log('🖥️ Using Electron Spotify API');
          ...
          //frontendLogger.log('🌐 Using SpotifyClient for albums');
          try {
            albumsResp = await spotifyClient.getArtistAlbums(deferredArtistId, { includeGroups: 'album,single', fetchAll: false, limit: 20 });
            //frontendLogger.log('✅ Albums response:', albumsResp);
          } catch (e) { 
            frontendLogger.error('❌ Albums request failed:', e);
            /* fallback failed */ 
          }
        }
        ...
        if (w.electron?.spotify?.searchPlaylists && currentTrack?.artists?.[0]?.name) {
          //frontendLogger.log('🖥️ Using Electron for playlist search');
          try {
          ...
          } catch (err) { frontendLogger.warn('Playlist proxy search failed', err); }
        } else if (currentTrack?.artists?.[0]?.name) {
          //frontendLogger.log('🌐 Using SpotifyClient for playlist search, artist:', currentTrack.artists[0].name);
          try {
            const pl = await spotifyClient.searchPlaylists(currentTrack.artists[0].name);
            //frontendLogger.log('✅ Playlists response:', pl);
            const plItems = (pl.items || (pl as any).playlists?.items || []).filter(Boolean);
            playlists = plItems.slice(0, 6) as any;
          } catch (err) { 
            frontendLogger.error('❌ Playlist search failed:', err);
            frontendLogger.warn('Playlist local search failed', err); 
          }
        }
        if (cancelled) return;
        //frontendLogger.log('🎯 Setting buckets - Singles:', singles.length, 'Albums:', realAlbums.length, 'Playlists:', playlists.length);
        setBuckets({ singles, albums: realAlbums, playlists, loading: false, fetched: true });
      } catch (e: any) {
        frontendLogger.error('❌ useArtistBuckets error:', e);
        if (!cancelled) {
          const msg = e?.message || 'Failed to load artist releases';
            ...
            frontendLogger.warn('Artist releases load error', e);
        }
      }
    }
    load();
