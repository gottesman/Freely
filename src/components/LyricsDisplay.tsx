...
const LyricsOverlay = ({ open, onClose, lyrics, title, synced }: LyricsOverlayProps) => {
  ...
  // Calculate effective position with sync offset and improved precision
  const effectivePosition = useMemo(() => {
    ...
    if (hasSynced && playing) {
      const timingInfo = {
        ...
        derivedPosition,
        syncOffsetMs,
        effectivePosition: offsetTime,
        isInterpolating: isInterpolatingRef.current
      };
    ...
    return offsetTime;
  }, [derivedPosition, position, syncOffsetMs, playing, hasSynced]);
    ...
    <div className={`np-lyrics-overlay ${open ? 'active' : ''}`} aria-hidden={!open} aria-label={t('lyrics.overlay')} role="dialog" onClick={handleBackdropClick}>
      <div className="np-lyrics-scroll" onClick={handleContentClick} ref={scrollRef}>
        <div className="np-lyrics-body">
          ...
          <button type="button" className="player-icons lyrics-btn lyrics-switch-btn" aria-label={t('lyrics.sswitchPanel')} onClick={onClose}>
            <span className="material-symbols-rounded filled">dock_to_right</span>
          </button>
          {hasSynced && (
            <button 
              type="button" 
