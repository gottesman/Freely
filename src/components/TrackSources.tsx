...
export default function TrackSources({ track, album, primaryArtist }: {
  ...
  const downloadSource = useCallback(async (source: any, sourceKey: string) => {
    ...
    try {
      ...
      if (downloadMap.has(sourceKey)) {
        ...
        const p = (async () => {
          try {
            await audioCache.downloadAndCache(track.id, sourceType, sourceHash, downloadUrl);
            ...
            while (Date.now() - startTs < totalTimeoutMs) {
              ...
              try {
                ...
                const isNowCached = await checkSourceCached(source, sourceKey);
                if (isNowCached) {
  ...
  }, [track?.id, checkSourceCached, handleSourceData]);
