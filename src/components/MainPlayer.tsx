...
export default function BottomPlayer({
  ...
  const codec = usePlaybackSelector(s => s.codec); // Codec/format from BASS
  const sampleRate = usePlaybackSelector(s => s.sampleRate); // Sample rate from BASS
  const bitsPerSample = usePlaybackSelector(s => s.bitsPerSample); // Bits per sample from BASS
  ...
  const isTrackLoading = useMemo(() => {
    ...
    if (currentTrack && (currentTrack as any).source && !noSource) {
      ...
      // Removed !playingFlag condition to fix edge case where switching tracks while playing
      // doesn't properly show loading state for the new track
    ...
    return false;
  }, [trackLoading, currentTrack, noSource, backendPosition, backendDuration, playbackUrl]);
  ...
  let getFormatDetails = useCallback(() => {
    if (!codec && !sampleRate && !bitsPerSample) return null;
    const sample = (sampleRate ? `${Math.round(sampleRate / 100)/10} kHz`: '') + (sampleRate && bitsPerSample ? ' • ' : '') + (bitsPerSample ? `${bitsPerSample}-bit` : '');
    return (
      <div className="format-info">
        {codec && formatIcon({ icon: codec })}
        <span title={sample} className={`custom-icon format-details ${(sampleRate > 96000 && bitsPerSample > 16) ? 'high' : (sampleRate > 41000 && bitsPerSample > 16) ? 'medium' : 'normal'
